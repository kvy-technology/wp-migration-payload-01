...
export async function up({ db, payload, req }: MigrateUpArgs): Promise<void> {
  ...
  `)
  await db.run(sql`INSERT INTO \`__new_posts\`("id", "title", "hero_image_id", "content", "category_id", "category_slug", "generate_slug", "slug", "updated_at", "created_at", "_status") SELECT "id", "title", "hero_image_id", "content", "category_id", "category_slug", "generate_slug", "slug", "updated_at", "created_at", "_status" FROM \`posts\`;`)
...
export async function down({ db, payload, req }: MigrateDownArgs): Promise<void> {
  ...
  `)
  await db.run(sql`INSERT INTO \`__new_posts\`("id", "title", "hero_image_id", "content", "category_id", "generate_slug", "slug", "updated_at", "created_at") SELECT "id", "title", "hero_image_id", "content", "category_id", "generate_slug", "slug", "updated_at", "created_at" FROM \`posts\`;`)
  await db.run(sql`DROP TABLE \`posts\`;`)
  await db.run(sql`ALTER TABLE \`__new_posts\` RENAME TO \`posts\`;`)
  await db.run(sql`PRAGMA foreign_keys=ON;`)
  await db.run(sql`CREATE INDEX \`posts_hero_image_idx\` ON \`posts\` (\`hero_image_id\`);`)
