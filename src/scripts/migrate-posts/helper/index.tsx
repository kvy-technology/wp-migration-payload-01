...
// Helper function to convert a table element to Lexical table node
...
// Helper function to extract tables and replace them with placeholders
...
// Helper function to check if a node or its children contain a placeholder
function containsPlaceholder(node: any, placeholders: string[]): boolean {
  if (!node || typeof node !== 'object') {
    return false
  }

  if (node.type === 'text' && typeof node.text === 'string') {
    return placeholders.some((placeholder) => node.text.includes(placeholder))
  }

  if (Array.isArray(node.children)) {
    return node.children.some((child: any) => containsPlaceholder(child, placeholders))
  }

  return false
}

// Helper function to find which placeholder is in a node
...
// Helper function to merge table nodes back into Lexical structure
...
export async function htmlToLexicalContent(
  ...
  const { htmlWithoutTables, tableNodes } = extractTablesFromHTML(html)
  console.log(`   Found ${tableNodes.length} table(s)`)

  // Step 2: Extract images from HTML (including from tables)
  console.log('📸 Extracting images from HTML...')
  const images = extractImagesFromHTML(html)
  console.log(`   Found ${images.length} image(s)`)

  // Step 3: Download and upload images to Payload
  ...
  // Step 4: Get the editor config
  const editorConfig = await editorConfigFactory.default({ config })

  // Step 5: Convert the HTML string (without tables) to Lexical JSON
  ...
  // Step 6: Merge table nodes back into the Lexical structure
  if (tableNodes.length > 0) {
    console.log('🔄 Merging table nodes into Lexical structure...')
    const mergedLexicalJSON = mergeTablesIntoLexical(lexicalJSON, tableNodes)

    // Step 7: Replace pending upload nodes with proper upload nodes
    console.log('🔄 Replacing pending upload nodes with media references...')
    const cleanLexicalJSON = await replacePendingUploads(mergedLexicalJSON, payload, imageMap)
